    ...
    match current_block_42 {
        11295390336478990698 => {
            *d
                ...
                ) = *s.offset((1 as libc::c_int - 2 as libc::c_int) as isize);
        }
        _ => {}
    };
}
...
unsafe extern "C" fn libzahl_memset(
    ...
    while i < n {
        *a.offset(i.wrapping_add(0 as libc::c_int as size_t) as isize) = v;
        *a.offset(i.wrapping_add(1 as libc::c_int as size_t) as isize) = v;
        *a.offset(i.wrapping_add(2 as libc::c_int as size_t) as isize) = v;
        *a.offset(i.wrapping_add(3 as libc::c_int as size_t) as isize) = v;
...
unsafe extern "C" fn zadd_impl_4(
    ...
    i = 0 as libc::c_int as size_t;
    loop {
        ac = ac.offset(4 as libc::c_int as isize);
        bc = bc.offset(4 as libc::c_int as isize);
        cc = cc.offset(4 as libc::c_int as isize);
        i = i.wrapping_add(4 as libc::c_int as size_t);
        if !(i <= n) {
            break;
        }
    ...
    match n & 3 as libc::c_int as size_t {
        3 => {
            asm!(
                "\n    addq $-1, {0}\n    movq -32({2}), {0}\n    adcq -32({3}), {0}\n    movq {0}, -32({1})\n    movq -24({2}), {0}\n    adcq -24({3}), {0}\n    movq {0}, -24({1})\n    movq -16({2}), {0}\n    adcq -16({3}), {0}\n    movq {0}, -16({1})\n    movq $1, {0}\n    jc 1f\n    movq $0, {0}\n 1:",
    ...
    while carry != 0 {
        let (fresh0, fresh1) = (*((*a).chars).offset(i as isize))
            .overflowing_add(1 as libc::c_int as libc::c_ulong);
...
unsafe extern "C" fn libzahl_zadd_unsigned(
    ...
    if (*a).alloced < size.wrapping_add(1 as libc::c_int as size_t) {
        libzahl_realloc(a, size.wrapping_add(1 as libc::c_int as size_t));
    }
    *((*a).chars).offset(size as isize) = 0 as libc::c_int as zahl_char_t;
    if a == b {
        ...
        zadd_impl_3(a, b, n);
    } else if ((*b).used > (*c).used) as libc::c_int as libc::c_long != 0 {
        libzahl_memcpy(
...
pub unsafe extern "C" fn zadd(mut a: *mut zahl, mut b: *mut zahl, mut c: *mut zahl) {
    ...
    } else if (zsignum(c) < 0 as libc::c_int) as libc::c_int as libc::c_long != 0 {
        zsub_unsigned(a, b, c);
    } else {
        libzahl_zadd_unsigned(a, b, c);
    };
