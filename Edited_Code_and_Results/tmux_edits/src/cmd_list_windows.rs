...
pub static mut cmd_list_windows_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"list-windows\0" as *const u8 as *const libc::c_char,
            alias: b"lsw\0" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_31 {
                    template: b"F:at:\0" as *const u8 as *const libc::c_char,
                    lower: 0 as libc::c_int,
                    upper: 0 as libc::c_int,
                };
                init
            },
            usage: b"[-a] [-F format] [-t target-session]\0" as *const u8
...
unsafe extern "C" fn cmd_list_windows_exec(
    ...
    if args_has(args, 'a' as i32 as u_char) != 0 {
        ...
        cmd_list_windows_session(self_0, (*item).target.s, item, 0 as libc::c_int);
    }
    return CMD_RETURN_NORMAL;
}
unsafe extern "C" fn cmd_list_windows_server(
    mut self_0: *mut cmd,
    mut item: *mut cmdq_item,
) {
...
unsafe extern "C" fn cmd_list_windows_session(
    ...
    if template.is_null() {
        match type_0 {
            0 => {
                template = b"#{window_index}: #{window_name}#{window_flags} (#{window_panes} panes) [#{window_width}x#{window_height}] [layout #{window_layout}] #{window_id}#{?window_active, (active),}\0"
                    as *const u8 as *const libc::c_char;
            }
            1 => {
                template = b"#{session_name}:#{window_index}: #{window_name}#{window_flags} (#{window_panes} panes) [#{window_width}x#{window_height}] \0"
                    as *const u8 as *const libc::c_char;
            }
            _ => {}
        }
    ...
    while !wl.is_null() {
        ...
        format_defaults(ft, 0 as *mut client, s, wl, 0 as *mut window_pane);
