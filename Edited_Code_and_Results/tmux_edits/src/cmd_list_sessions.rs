...
pub static mut cmd_list_sessions_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"list-sessions\0" as *const u8 as *const libc::c_char,
            alias: b"ls\0" as *const u8 as *const libc::c_char,
            args: {
...
unsafe extern "C" fn cmd_list_sessions_exec(
    mut self_0: *mut cmd,
    ...
    if template.is_null() {
        template = b"#{session_name}: #{session_windows} windows (created #{t:session_created}) [#{session_width}x#{session_height}]#{?session_grouped, (group ,}#{session_group}#{?session_grouped,),}#{?session_attached, (attached),}\0"
