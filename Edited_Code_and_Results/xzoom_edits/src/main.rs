// #![feature(extern_types)] // Already commented out, no change needed
use libc::{alarm, usleep};
use ::libc;

extern "C" {
    // Remove these extern type declarations; libc::FILE handles them
    // pub type _IO_wide_data;
    // pub type _IO_codecvt;
    // pub type _IO_marker;
    // Remove these X11 extern types; we'll define them as c_void below
    // pub type _XGC;
    // pub type _XDisplay;
    // pub type _XPrivate;
    // pub type _XrmHashBucketRec;
    static mut stderr: *mut libc::FILE; // Change FILE to libc::FILE
    fn fprintf(_: *mut libc::FILE, _: *const libc::c_char, _: ...) -> libc::c_int; // Change FILE to libc::FILE
...
// Define X11 types as opaque pointers to replace removed extern types
pub type _XGC = libc::c_void;
pub type _XDisplay = libc::c_void;
pub type _XPrivate = libc::c_void;
pub type _XrmHashBucketRec = libc::c_void;

pub type size_t = libc::c_ulong; // Keep for now, but could remove later
pub type __off_t = libc::c_long; // Keep for now, but could remove later
pub type __off64_t = libc::c_long; // Keep for now, but could remove later

// Remove _IO_FILE struct; use libc::FILE instead
// #[derive(Copy, Clone)]
// #[repr(C)]
// pub struct _IO_FILE {
...
pub unsafe extern "C" fn scale8() {
    ...
    loop {
        ...
        p1 = &mut *((**ximage.as_mut_ptr().offset(1 as libc::c_int as isize)).data)
            .offset(
        ...
        p1_save = p1;
        p2 = &mut *((**ximage.as_mut_ptr().offset(0 as libc::c_int as isize)).data)
            .offset(
...
unsafe fn main_0(
            ...
            as libc::c_int == '=' as i32
        {
            dest_geom_mask = XParseGeometry(
                *argv.offset(0 as libc::c_int as isize),
                &mut xpos,
                &mut ypos,
                &mut *width.as_mut_ptr().offset(1 as libc::c_int as isize)
                    as *mut libc::c_int as *mut libc::c_uint,
                &mut *height.as_mut_ptr().offset(1 as libc::c_int as isize)
                    as *mut libc::c_int as *mut libc::c_uint,
            );
        } else if strcmp(
            *argv.offset(0 as libc::c_int as isize),
        ...
        ) == 0
        {
            ...
            source_geom_mask = XParseGeometry(
                *argv.offset(0 as libc::c_int as isize),
                &mut xgrab,
                &mut ygrab,
                &mut *width.as_mut_ptr().offset(0 as libc::c_int as isize)
                    as *mut libc::c_int as *mut libc::c_uint,
                &mut *height.as_mut_ptr().offset(0 as libc::c_int as isize)
                    as *mut libc::c_int as *mut libc::c_uint,
            );
        } else if strcmp(
            *argv.offset(0 as libc::c_int as isize),
    ...
    scr = &mut *((*(dpy as _XPrivDisplay)).screens)
    ...
    XStringListToTextProperty(&mut progname, 1 as libc::c_int, &mut str);
    ch = XAllocClassHint();
    ...
    if isdock != 0 {
        ...
        XChangeProperty(
            ...
            32 as libc::c_int,
            0 as libc::c_int,
            &mut wm_dock as *mut Atom as *mut libc::c_uchar,
            1 as libc::c_int,
        );
    }
    ...
    status = XSetWMProtocols(dpy, win, &mut wm_delete_window, 1 as libc::c_int);
    set_title = 1 as libc::c_int;
