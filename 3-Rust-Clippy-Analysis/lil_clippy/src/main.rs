...
unsafe extern "C" fn do_system(
    ...
    let mut cmdlen: libc::c_int = 0 as libc::c_int;
    let mut i: size_t = 0;
    ...
    while i < argc {
        ...
        cmd = realloc(cmd as *mut libc::c_void, (cmdlen as size_t).wrapping_add(len))
            as *mut libc::c_char;
        memcpy(
        ...
        cmdlen = (cmdlen as size_t).wrapping_add(len) as libc::c_int as libc::c_int;
        i = i.wrapping_add(1);
        i;
    }
    ...
    *cmd.offset(cmdlen as isize) = 0 as libc::c_int as libc::c_char;
    ...
    if !p.is_null() {
        ...
        loop {
            ...
            size = size.wrapping_add(bytes as size_t);
        }
        retval = realloc(
            retval as *mut libc::c_void,
...
unsafe extern "C" fn fnc_system(
    ...
    let mut sargv: *mut *const libc::c_char = malloc(
        (::core::mem::size_of::<*mut libc::c_char>() as libc::c_ulong)
            .wrapping_mul(argc.wrapping_add(1 as libc::c_int as size_t)),
...
unsafe extern "C" fn fnc_readline(
    ...
    let mut size: size_t = 64 as libc::c_int as size_t;
    let mut buffer: *mut libc::c_char = malloc(size) as *mut libc::c_char;
    ...
    loop {
        ...
        if len < size {
            size = size.wrapping_add(64 as libc::c_int as size_t);
...
unsafe extern "C" fn repl() -> libc::c_int {
    ...
    while running != 0 {
        ...
        let mut strres: *const libc::c_char = 0 as *const libc::c_char;
...
unsafe extern "C" fn nonint(
    ...
    let mut result: lil_value_t = 0 as *mut _lil_value_t;
    let mut tmpcode: *mut libc::c_char = 0 as *mut libc::c_char;
