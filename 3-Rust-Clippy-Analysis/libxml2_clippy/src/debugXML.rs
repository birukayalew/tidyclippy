...
unsafe extern "C" fn xmlCtxtGenericNodeCheck(
    ...
    match (*node).type_0 as libc::c_uint {
        ...
        5 | 6 | 10 | 11 | 12 | 14 | 15 | 16 | 17 | 18 | 19 | 20 | 21 | 9 | 13 | _ => {}
    };
}
unsafe extern "C" fn xmlCtxtDumpString(
    ...
    while i < 40 as libc::c_int {
        if *str.offset(i as isize) as libc::c_int == 0 as libc::c_int {
            return
        } else if *str.offset(i as isize) as libc::c_int == 0x20 as libc::c_int
...
pub unsafe extern "C" fn xmlDebugDumpAttr(
    ...
    let mut ctxt: xmlDebugCtxt = _xmlDebugCtxt {
...
unsafe extern "C" fn xmlShellRegisterNamespace(
    ...
    while !next.is_null() {
        ...
        let fresh0 = next;
        next = next.offset(1);
        *fresh0 = '\0' as i32 as xmlChar;
        href = next;
        next = xmlStrchr(next, ' ' as i32 as xmlChar) as *mut xmlChar;
        if !next.is_null() {
            let fresh1 = next;
            next = next.offset(1);
            *fresh1 = '\0' as i32 as xmlChar;
        }
...
pub unsafe extern "C" fn xmlShellLoad(
    ...
    if !doc.is_null() {
        ...
        (*ctxt).filename = xmlCanonicPath(filename as *mut xmlChar) as *mut libc::c_char;
    } else {
        return -(1 as libc::c_int)
    }
    return 0 as libc::c_int;
...
pub unsafe extern "C" fn xmlShellValidate(
    ...
    vctxt.userData = stderr as *mut libc::c_void;
    vctxt
...
pub unsafe extern "C" fn xmlShell(
    ...
    let mut prompt: [libc::c_char; 500] = *::core::mem::transmute::<
        &[u8; 500],
        &mut [libc::c_char; 500],
    >(
    ...
    ctxt = xmlMalloc
        .expect(
            "non-null function pointer",
        )(::core::mem::size_of::<xmlShellCtxt>() as libc::c_ulong) as xmlShellCtxtPtr;
    if ctxt.is_null() {
        return;
    }
    ...
    (*ctxt).input = input;
    (*ctxt).output = output;
    (*ctxt).filename = xmlStrdup(filename as *mut xmlChar) as *mut libc::c_char;
    ...
    loop {
        ...
        prompt[(::core::mem::size_of::<[libc::c_char; 500]>() as libc::c_ulong)
        ...
        ) == 0
        {
            ...
            || strcmp(command.as_mut_ptr(), b"dir\0" as *const u8 as *const libc::c_char)
                == 0
        {
