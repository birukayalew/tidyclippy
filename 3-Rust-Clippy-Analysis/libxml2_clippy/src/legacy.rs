...
pub unsafe extern "C" fn xmlGetFeaturesList(
    ...
    ret = (::core::mem::size_of::<[*const libc::c_char; 42]>() as libc::c_ulong)
...
pub unsafe extern "C" fn xmlSetFeature(
        ...
        == 0
    {
        ...
        (*ctxt).userData = *(value as *mut *mut libc::c_void);
    } else if strcmp(name, b"is html\0" as *const u8 as *const libc::c_char) == 0 {
    ...
    {
        (*(*ctxt).sax).hasInternalSubset = *(value as *mut hasInternalSubsetSAXFunc);
    } else if strcmp(
        name,
        b"SAX function hasExternalSubset\0" as *const u8 as *const libc::c_char,
    ) == 0
    {
        (*(*ctxt).sax).hasExternalSubset = *(value as *mut hasExternalSubsetSAXFunc);
    } else if strcmp(
        name,
        b"SAX function resolveEntity\0" as *const u8 as *const libc::c_char,
    ) == 0
    ...
    {
        (*(*ctxt).sax).unparsedEntityDecl = *(value as *mut unparsedEntityDeclSAXFunc);
    } else if strcmp(
        name,
        b"SAX function setDocumentLocator\0" as *const u8 as *const libc::c_char,
    ) == 0
    {
        (*(*ctxt).sax).setDocumentLocator = *(value as *mut setDocumentLocatorSAXFunc);
    } else if strcmp(
        name,
        b"SAX function startDocument\0" as *const u8 as *const libc::c_char,
    ) == 0
    ...
    {
        (*(*ctxt).sax)
            .processingInstruction = *(value as *mut processingInstructionSAXFunc);
    } else if strcmp(name, b"SAX function comment\0" as *const u8 as *const libc::c_char)
        == 0
    {
    ...
    {
        (*(*ctxt).sax).getParameterEntity = *(value as *mut getParameterEntitySAXFunc);
    } else if strcmp(
        name,
        b"SAX function cdataBlock\0" as *const u8 as *const libc::c_char,
    ) == 0
