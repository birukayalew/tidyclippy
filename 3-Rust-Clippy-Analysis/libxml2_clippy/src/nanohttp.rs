...
unsafe extern "C" fn xmlNanoHTTPScanURL(
    ...
    if !((*ctxt).query).is_null() {
        ...
        (*ctxt).query = 0 as *mut libc::c_char;
    }
    if URL.is_null() {
        return;
    }
    ...
    if !((*uri).server).is_null() && *(*uri).server as libc::c_int == '[' as i32 {
        ...
        {
            (*ctxt)
                .hostname = xmlCharStrndup(
                ((*uri).server).offset(1 as libc::c_int as isize),
                len - 2 as libc::c_int,
            ) as *mut libc::c_char;
        } else {
            (*ctxt)
                .hostname = xmlMemStrdup
                .expect("non-null function pointer")((*uri).server);
        }
    } else {
        (*ctxt)
            .hostname = xmlMemStrdup.expect("non-null function pointer")((*uri).server);
    }
    if !((*uri).path).is_null() {
...
unsafe extern "C" fn xmlNanoHTTPNewCtxt(
    ...
    ret = xmlMalloc
        .expect(
            "non-null function pointer",
        )(::core::mem::size_of::<xmlNanoHTTPCtxt>() as libc::c_ulong)
...
unsafe extern "C" fn xmlNanoHTTPScanAnswer(
    ...
    ) == 0
    {
        ...
        if *cur as libc::c_int == '/' as i32 {
            ...
            (*ctxt)
                .location = xmlStrcat(tmp_loc, cur as *const xmlChar)
                as *mut libc::c_char;
        } else {
    ...
    ) == 0
    {
        ...
        ) == 0
        {
            ...
            (*ctxt)
                ...
                .expect(
                    "non-null function pointer",
                )(::core::mem::size_of::<z_stream>() as libc::c_ulong) as *mut z_stream;
            if !((*ctxt).strm).is_null() {
...
unsafe extern "C" fn xmlNanoHTTPConnectAttempt(mut addr: *mut sockaddr) -> libc::c_int {
    ...
    if (*addr).sa_family as libc::c_int == 10 as libc::c_int {
        ...
        addrlen = ::core::mem::size_of::<sockaddr_in6>() as libc::c_ulong as libc::c_int;
    } else {
        s = socket(
            2 as libc::c_int,
            SOCK_STREAM as libc::c_int,
    ...
    if have_ipv6() != 0 {
        ...
        while !res.is_null() {
            if (*res).ai_family == 2 as libc::c_int {
                ...
                current_block_33 = 9828876828309294594;
                ...
                current_block_33 = 13109137661213826276;
            }
        ...
        if h.is_null() {
            ...
            match *__h_errno_location() {
                1 => {
                    h_err_txt = b"Authoritive host not found\0" as *const u8
                        as *const libc::c_char;
                }
                2 => {
                    h_err_txt = b"Non-authoritive host not found or server failure.\0"
...
pub unsafe extern "C" fn xmlNanoHTTPRead(
    ...
    if (*ctxt).usesGzip == 1 as libc::c_int {
        ...
        (*(*ctxt).strm)
            ...
            as uInt;
        while (*(*ctxt).strm).avail_out > 0 as libc::c_int as uInt
            && ((*(*ctxt).strm).avail_in > 0 as libc::c_int as uInt
                || xmlNanoHTTPRecv(ctxt) > 0 as libc::c_int)
        {
            ...
            orig_avail_in = (*(*ctxt).strm).avail_in as libc::c_int;
            (*(*ctxt).strm)
                .next_in = ((*ctxt).inrptr).offset(bytes_read as isize) as *mut xmlChar;
            z_ret = inflate((*ctxt).strm, 0 as libc::c_int);
            bytes_read = (bytes_read as uInt)
                .wrapping_add(
                    (orig_avail_in as uInt).wrapping_sub((*(*ctxt).strm).avail_in),
        ...
        return (len as uInt).wrapping_sub((*(*ctxt).strm).avail_out) as libc::c_int;
    }
...
pub unsafe extern "C" fn xmlNanoHTTPMethodRedir(
    ...
    loop {
        if redirURL.is_null() {
            ...
            (*ctxt)
                .location = xmlMemStrdup.expect("non-null function pointer")(redirURL);
        }
        if ((*ctxt).protocol).is_null()
