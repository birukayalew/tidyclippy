...
pub static mut cmd_rename_session_entry: cmd_entry = unsafe {
    {
        let mut init = cmd_entry {
            name: b"rename-session\0" as *const u8 as *const libc::c_char,
            alias: b"rename\0" as *const u8 as *const libc::c_char,
            args: {
            ...
            usage: b"[-t target-session] new-name\0" as *const u8 as *const libc::c_char,
...
unsafe extern "C" fn cmd_rename_session_exec(
    ...
    let mut c: *mut client = cmd_find_client(
        item,
    ...
    notify_session(b"session-renamed\0" as *const u8 as *const libc::c_char, s);
    return CMD_RETURN_NORMAL;
}
