...
unsafe extern "C" fn bsPutUChar(mut bs: *mut BitStream, mut c: UChar) {
    ...
    while i >= 0 as libc::c_int {
        bsPutBit(bs, (c as UInt32 >> i & 0x1 as libc::c_int as UInt32) as Int32);
        i -= 1;
        i;
    }
}
unsafe extern "C" fn bsPutUInt32(mut bs: *mut BitStream, mut c: UInt32) {
    ...
    i = 31 as libc::c_int;
    while i >= 0 as libc::c_int {
        bsPutBit(bs, (c >> i & 0x1 as libc::c_int as UInt32) as Int32);
        i -= 1;
        i;
    }
}
...
unsafe fn main_0(mut argc: Int32, mut argv: *mut *mut Char) -> Int32 {
        ...
        if b == 2 as libc::c_int {
            if bitsRead >= bStart[currBlock as usize]
                && bitsRead.wrapping_sub(bStart[currBlock as usize])
                    >= 40 as libc::c_int as MaybeUInt64
            {
                bEnd[currBlock
                    as usize] = bitsRead.wrapping_sub(1 as libc::c_int as MaybeUInt64);
            ...
            buffLo = buffLo << 1 as libc::c_int | (b & 1 as libc::c_int) as UInt32;
            if (buffHi & 0xffff as libc::c_int as UInt32) as libc::c_ulong
                == 0x3141 as libc::c_ulong
                && buffLo as libc::c_ulong == 0x59265359 as libc::c_ulong
                || (buffHi & 0xffff as libc::c_int as UInt32) as libc::c_ulong
                    == 0x1772 as libc::c_ulong
                    && buffLo as libc::c_ulong == 0x45385090 as libc::c_ulong
            {
                if bitsRead > 49 as libc::c_int as MaybeUInt64 {
                    bEnd[currBlock
                        as usize] = bitsRead
                        .wrapping_sub(49 as libc::c_int as MaybeUInt64);
                } else {
                ...
                if currBlock > 0 as libc::c_int
                    && (bEnd[currBlock as usize])
                        ...
                        >= 130 as libc::c_int as MaybeUInt64
                {
                    fprintf(
                        stderr,
    ...
    while 1 as libc::c_int as Bool != 0 {
        ...
        buffLo = buffLo << 1 as libc::c_int | (b & 1 as libc::c_int) as UInt32;
        if bitsRead
            == (47 as libc::c_int as MaybeUInt64).wrapping_add(rbStart[wrBlock as usize])
        {
        ...
        bitsRead;
        if bitsRead
            == (rbEnd[wrBlock as usize]).wrapping_add(1 as libc::c_int as MaybeUInt64)
        {
            if !outFile.is_null() {
