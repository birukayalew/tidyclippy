...
#[no_mangle]
pub unsafe extern "C" fn xmlWriterErrMsg(
...
pub unsafe extern "C" fn xmlNewTextWriter(
    ...
    ret = xmlMalloc
        ...
        )(::core::mem::size_of::<xmlTextWriter>() as libc::c_ulong) as xmlTextWriterPtr;
    if ret.is_null() {
        xmlWriterErrMsg(
    ...
    memset(
        ...
        0 as libc::c_int,
        ::core::mem::size_of::<xmlTextWriter>() as libc::c_ulong,
    );
    (*ret)
        .nodes = xmlListCreate(
    ...
    (*ret).out = out;
    (*ret).ichar = xmlStrdup(b" \0" as *const u8 as *const libc::c_char as *mut xmlChar);
...
pub unsafe extern "C" fn xmlNewTextWriterDoc(
    ...
    let mut ret: xmlTextWriterPtr = 0 as *mut xmlTextWriter;
    let mut saxHandler: xmlSAXHandler = _xmlSAXHandler {
        internalSubset: None,
        isStandalone: None,
...
pub unsafe extern "C" fn xmlTextWriterStartDocument(
    ...
    if !encoder.is_null() {
        if ((*(*writer).out).conv).is_null() {
            (*(*writer).out).conv = xmlBufCreateSize(4000 as libc::c_int as size_t);
        }
        xmlCharEncOutput((*writer).out, 1 as libc::c_int);
        if !((*writer).doc).is_null() && ((*(*writer).doc).encoding).is_null() {
            (*(*writer).doc)
                .encoding = xmlStrdup((*(*(*writer).out).encoder).name as *mut xmlChar);
        }
    } else {
        (*(*writer).out).conv = 0 as xmlBufPtr;
    }
    sum = 0 as libc::c_int;
...
pub unsafe extern "C" fn xmlTextWriterStartComment(
    ...
    p = xmlMalloc
        .expect(
            "non-null function pointer",
        )(::core::mem::size_of::<xmlTextWriterStackEntry>() as libc::c_ulong)
...
pub unsafe extern "C" fn xmlTextWriterStartElement(
    ...
    let mut lk: xmlLinkPtr = 0 as *mut xmlLink;
    let mut p: *mut xmlTextWriterStackEntry = 0 as *mut xmlTextWriterStackEntry;
    if writer.is_null() || name.is_null() || *name as libc::c_int == '\0' as i32 {
        return -(1 as libc::c_int);
    }
    sum = 0 as libc::c_int;
    lk = xmlListFront((*writer).nodes);
        ...
        if !p.is_null() {
            ...
            match (*p).state as libc::c_uint {
                ...
                2 => {
                    ...
                    sum += count;
                    current_block_20 = 12336535586471661431;
                }
                1 => {
                    current_block_20 = 12336535586471661431;
                }
                0 | _ => {
                    current_block_20 = 13797916685926291137;
                }
            }
...
pub unsafe extern "C" fn xmlTextWriterStartElementNS(
    ...
    if !namespaceURI.is_null() {
        let mut p: *mut xmlTextWriterNsStackEntry = xmlMalloc
            .expect(
                "non-null function pointer",
            )(::core::mem::size_of::<xmlTextWriterNsStackEntry>() as libc::c_ulong)
...
pub unsafe extern "C" fn xmlTextWriterEndElement(
    ...
    match (*p).state as libc::c_uint {
        2 => {
            ...
            sum += count;
            current_block_50 = 17614209453473539298;
        }
        1 => {
            current_block_50 = 17614209453473539298;
        }
        3 => {
            ...
            sum += count;
            current_block_50 = 4090602189656566074;
        }
        _ => return -(1 as libc::c_int),
    }
    match current_block_50 {
        17614209453473539298 => {
            count = xmlTextWriterOutputNSDecl(writer);
            if count < 0 as libc::c_int {
                return -(1 as libc::c_int);
            }
...
pub unsafe extern "C" fn xmlTextWriterFullEndElement(
    ...
    match (*p).state as libc::c_uint {
        2 => {
            ...
            sum += count;
            current_block_41 = 6043727168983170021;
        }
        1 => {
            current_block_41 = 6043727168983170021;
        }
        3 => {
            current_block_41 = 17862714266112244355;
        }
        _ => return -(1 as libc::c_int),
    }
...
pub unsafe extern "C" fn xmlTextWriterStartAttributeNS(
    ...
    if !namespaceURI.is_null() {
        let mut nsentry: xmlTextWriterNsStackEntry = _xmlTextWriterNsStackEntry {
...
pub unsafe extern "C" fn xmlTextWriterStartPI(
    ...
    let mut lk: xmlLinkPtr = 0 as *mut xmlLink;
    let mut p: *mut xmlTextWriterStackEntry = 0 as *mut xmlTextWriterStackEntry;
    if writer.is_null() || target.is_null() || *target as libc::c_int == '\0' as i32 {
        return -(1 as libc::c_int);
    }
    if xmlStrcasecmp(
        target,
        ...
        if !p.is_null() {
            ...
            match (*p).state as libc::c_uint {
                2 => {
                    ...
                    sum += count;
                    current_block_24 = 8049013476527395938;
                }
                1 => {
                    current_block_24 = 8049013476527395938;
                }
...
pub unsafe extern "C" fn xmlTextWriterStartCDATA(
    ...
    if !lk.is_null() {
        ...
        if !p.is_null() {
            ...
            match (*p).state as libc::c_uint {
                0 | 3 | 4 | 5 => {
                    current_block_20 = 2838571290723028321;
                }
                2 => {
                    count = xmlTextWriterEndAttribute(writer);
                    if count < 0 as libc::c_int {
                    ...
                    sum += count;
                    current_block_20 = 8476290697041545516;
                }
                1 => {
                    current_block_20 = 8476290697041545516;
                }
    ...
    while loop_0 != 0 {
        ...
        match (*p).state as libc::c_uint {
            8 => {
                ...
                sum += count;
                current_block_25 = 6656740157277691119;
            }
            7 => {
                current_block_25 = 6656740157277691119;
            }
...
pub unsafe extern "C" fn xmlTextWriterWriteDTDInternalEntity(
    ...
    let mut count: libc::c_int = 0;
    let mut sum: libc::c_int = 0;
    if name.is_null() || *name as libc::c_int == '\0' as i32 || content.is_null() {
        return -(1 as libc::c_int);
    }
    sum = 0 as libc::c_int;
...
unsafe extern "C" fn xmlTextWriterStartDocumentCallback(mut ctx: *mut libc::c_void) {
    ...
    if (*ctxt).html != 0 {
        if ((*ctxt).myDoc).is_null() {
            (*ctxt).myDoc = htmlNewDocNoDtD(0 as *const xmlChar, 0 as *const xmlChar);
        }
        if ((*ctxt).myDoc).is_null() {
        ...
        if !doc.is_null() {
            if ((*doc).children).is_null() {
                if !((*ctxt).encoding).is_null() {
                    ...
                    (*doc).encoding = 0 as *const xmlChar;
                }
    ...
    {
        (*(*ctxt).myDoc)
            .URL = xmlCanonicPath((*(*ctxt).input).filename as *const xmlChar);
        if ((*(*ctxt).myDoc).URL).is_null() {
            (*(*ctxt).myDoc)
                .URL = xmlStrdup((*(*ctxt).input).filename as *const xmlChar);
        }
    }
}
#[no_mangle]
...
unsafe extern "C" fn xmlTextWriterHandleStateDependencies(
    ...
    sum = 0 as libc::c_int;
    extra[2 as libc::c_int as usize] = '\0' as i32 as libc::c_char;
    ...
    if *extra.as_mut_ptr() as libc::c_int != '\0' as i32 {
