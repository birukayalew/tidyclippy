...
static mut IOerr: [*const libc::c_char; 57] = [
    ...
    b"adddress in use\0" as *const u8 as *const libc::c_char,
    b"already in use\0" as *const u8 as *const libc::c_char,
    b"unknown address familly\0" as *const u8 as *const libc::c_char,
];
...
pub unsafe extern "C" fn __xmlIOErr(
    ...
    if idx as libc::c_ulong
        >= (::core::mem::size_of::<[*const libc::c_char; 57]>() as libc::c_ulong)
...
pub unsafe extern "C" fn xmlCheckFilename(mut path: *const libc::c_char) -> libc::c_int {
    ...
    if stat_buffer.st_mode & 0o170000 as libc::c_int as __mode_t
...
unsafe extern "C" fn xmlFileOpen_real(
    ...
    fd = fopen(path, b"r\0" as *const u8 as *const libc::c_char);
    if fd.is_null() {
...
unsafe extern "C" fn xmlCreateZMemBuff(
    ...
    buff = xmlMalloc
        ...
        )(::core::mem::size_of::<xmlZMemBuff>() as libc::c_ulong) as xmlZMemBuffPtr;
    if buff.is_null() {
    ...
    memset(
        ...
        0 as libc::c_int,
        ::core::mem::size_of::<xmlZMemBuff>() as libc::c_ulong,
    );
    (*buff).size = 32768 as libc::c_int as libc::c_ulong;
    (*buff)
        .zbuff = xmlMalloc.expect("non-null function pointer")((*buff).size)
        as *mut libc::c_uchar;
...
unsafe extern "C" fn xmlZMemBuffExtend(
    ...
    if buff.is_null() {
        return -(1 as libc::c_int)
    } else if ext_amt == 0 as libc::c_int as size_t {
        return 0 as libc::c_int
    }
...
unsafe extern "C" fn xmlZMemBuffAppend(
    ...
    (*buff).zctrl.next_in = src as *mut libc::c_uchar;
    while (*buff).zctrl.avail_in > 0 as libc::c_int as uInt {
        min_accept = ((*buff).zctrl.avail_in / 5 as libc::c_int as uInt) as size_t;
...
pub unsafe extern "C" fn xmlIOHTTPOpenW(
    ...
    ctxt = xmlMalloc
        .expect(
            "non-null function pointer",
        )(::core::mem::size_of::<xmlIOHTTPWriteCtxt>() as libc::c_ulong)
    ...
    (*ctxt).uri = xmlStrdup(post_uri as *const xmlChar) as *mut libc::c_char;
    if ((*ctxt).uri).is_null() {
    ...
    if compression > 0 as libc::c_int && compression <= 9 as libc::c_int {
        ...
        (*ctxt)
            .doc_buff = xmlAllocOutputBufferInternal(0 as xmlCharEncodingHandlerPtr)
            as *mut libc::c_void;
    }
    if ((*ctxt).doc_buff).is_null() {
        xmlFreeHTTPWriteCtxt(ctxt);
...
pub unsafe extern "C" fn xmlAllocParserInputBuffer(
    ...
    ret = xmlMalloc
        .expect(
            "non-null function pointer",
        )(::core::mem::size_of::<xmlParserInputBuffer>() as libc::c_ulong)
    ...
    if !((*ret).encoder).is_null() {
        (*ret)
            .raw = xmlBufCreateSize(
            (2 as libc::c_int * *__xmlDefaultBufferSize()) as size_t,
        );
    } else {
        (*ret).raw = 0 as xmlBufPtr;
    }
    (*ret).readcallback = None;
    (*ret).closecallback = None;
    (*ret).context = 0 as *mut libc::c_void;
...
pub unsafe extern "C" fn xmlAllocOutputBuffer(
    ...
    if xmlBufGetAllocationScheme((*ret).buffer) == XML_BUFFER_ALLOC_EXACT as libc::c_int
    {
        xmlBufSetAllocationScheme((*ret).buffer, XML_BUFFER_ALLOC_DOUBLEIT);
    }
    (*ret).encoder = encoder;
    if !encoder.is_null() {
        (*ret).conv = xmlBufCreateSize(4000 as libc::c_int as size_t);
        if ((*ret).conv).is_null() {
            xmlFree.expect("non-null function pointer")(ret as *mut libc::c_void);
            return 0 as xmlOutputBufferPtr;
        }
...
pub unsafe extern "C" fn xmlParserInputBufferCreateFile(
    ...
    ret = xmlAllocParserInputBuffer(enc);
    if !ret.is_null() {
        (*ret).context = file as *mut libc::c_void;
        (*ret)
            .readcallback = Some(
            xmlFileRead
                as unsafe extern "C" fn(
...
pub unsafe extern "C" fn xmlParserInputBufferCreateFd(
    ...
    ret = xmlAllocParserInputBuffer(enc);
    if !ret.is_null() {
        (*ret).context = fd as ptrdiff_t as *mut libc::c_void;
        (*ret)
            .readcallback = Some(
            xmlFdRead
                as unsafe extern "C" fn(
...
pub unsafe extern "C" fn xmlParserInputBufferCreateMem(
    ...
    ret = xmlAllocParserInputBuffer(enc);
    if !ret.is_null() {
        (*ret).context = mem as *mut libc::c_void;
        (*ret)
            .readcallback = Some(
...
pub unsafe extern "C" fn xmlParserInputBufferCreateStatic(
    ...
    (*ret).buffer = xmlBufCreateStatic(mem as *mut libc::c_void, size as size_t);
    if ((*ret).buffer).is_null() {
        xmlFree.expect("non-null function pointer")(ret as *mut libc::c_void);
        return 0 as xmlParserInputBufferPtr;
    }
...
pub unsafe extern "C" fn xmlParserInputBufferPush(
    ...
    if !((*in_0).encoder).is_null() {
        ...
        (*in_0)
            .rawconsumed = ((*in_0).rawconsumed)
            .wrapping_add((use_0 as size_t).wrapping_sub(xmlBufUse((*in_0).raw)));
    } else {
        nbchars = len;
...
pub unsafe extern "C" fn xmlParserInputBufferGrow(
    ...
    if xmlBufAvail((*in_0).buffer) <= 0 as libc::c_int as size_t {
...
pub unsafe extern "C" fn xmlOutputBufferWrite(
    ...
    loop {
        ...
        if !((*out).encoder).is_null() {
            ...
            if xmlBufUse((*out).buffer) < 4000 as libc::c_int as size_t && chunk == len {
                break;
            }
    ...
    while in_0 < inend && out < outend {
        if *in_0 as libc::c_int == '<' as i32 {
            ...
            *fresh4 = '&' as i32 as libc::c_uchar;
            let fresh5 = out;
            out = out.offset(1);
            *fresh5 = 'l' as i32 as libc::c_uchar;
            let fresh6 = out;
            out = out.offset(1);
            *fresh6 = 't' as i32 as libc::c_uchar;
            let fresh7 = out;
            out = out.offset(1);
            ...
            *fresh8 = '&' as i32 as libc::c_uchar;
            let fresh9 = out;
            out = out.offset(1);
            *fresh9 = 'g' as i32 as libc::c_uchar;
            let fresh10 = out;
            out = out.offset(1);
            *fresh10 = 't' as i32 as libc::c_uchar;
            let fresh11 = out;
            out = out.offset(1);
            ...
            *fresh13 = 'a' as i32 as libc::c_uchar;
            let fresh14 = out;
            out = out.offset(1);
            *fresh14 = 'm' as i32 as libc::c_uchar;
            let fresh15 = out;
            out = out.offset(1);
            *fresh15 = 'p' as i32 as libc::c_uchar;
            let fresh16 = out;
            out = out.offset(1);
            ...
            *fresh18 = '#' as i32 as libc::c_uchar;
            let fresh19 = out;
            out = out.offset(1);
            *fresh19 = '1' as i32 as libc::c_uchar;
            let fresh20 = out;
            out = out.offset(1);
            *fresh20 = '3' as i32 as libc::c_uchar;
            let fresh21 = out;
            out = out.offset(1);
...
pub unsafe extern "C" fn xmlOutputBufferWriteEscape(
    ...
    loop {
        oldwritten = written;
        cons = len;
        chunk = (xmlBufAvail((*out).buffer)).wrapping_sub(1 as libc::c_int as size_t)
            as libc::c_int;
        if chunk < 40 as libc::c_int {
            ...
            if !((*out).encoder).is_null() {
                ...
                if xmlBufUse((*out).buffer) < 4000 as libc::c_int as size_t
                    && cons == len
                {
                    break;
                }
            ...
            if ((*out).writecallback).is_some() {
                ...
                (*out).written += ret;
            } else if xmlBufAvail((*out).buffer) < 4000 as libc::c_int as size_t {
        ...
        if code >= 400 as libc::c_int {
            ...
            if !redir.is_null() {
                ...
                if !((*ret).directory).is_null() {
                    ...
                    (*ret).directory = 0 as *const libc::c_char;
                }
                (*ret)
                    .filename = xmlStrdup(redir as *const xmlChar) as *mut libc::c_char;
            }
        }
    }
    return ret;
