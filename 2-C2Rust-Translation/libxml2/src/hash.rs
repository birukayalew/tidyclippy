...
pub unsafe extern "C" fn xmlHashCreate(mut size: libc::c_int) -> xmlHashTablePtr {
    ...
    table = xmlMalloc
        ...
        )(::core::mem::size_of::<xmlHashTable>() as libc::c_ulong) as xmlHashTablePtr;
    if !table.is_null() {
        ...
        (*table)
            ...
            (size as libc::c_ulong)
                .wrapping_mul(::core::mem::size_of::<xmlHashEntry>() as libc::c_ulong),
        ...
        if !((*table).table).is_null() {
            memset(
                ...
                (size as libc::c_ulong)
                    .wrapping_mul(
                        ::core::mem::size_of::<xmlHashEntry>() as libc::c_ulong,
                    ),
            );
    ...
    while i < oldsize {
        ...
        while !iter.is_null() {
            ...
            if (*((*table).table).offset(key as isize)).valid == 0 as libc::c_int {
                ...
                let ref mut fresh10 = (*((*table).table).offset(key as isize)).next;
                *fresh10 = 0 as *mut _xmlHashEntry;
                ...
                let ref mut fresh11 = (*((*table).table).offset(key as isize)).next;
                *fresh11 = iter;
            }
            iter = next;
        }
        ...
        while i < (*table).size && nbElems > 0 as libc::c_int {
            ...
            if !((*iter).valid == 0 as libc::c_int) {
                ...
                while !iter.is_null() {
                    ...
                    (*iter).payload = 0 as *mut libc::c_void;
                    if inside_table == 0 {
                        xmlFree
                            .expect(
                                "non-null function pointer",
    ...
    if (*((*table).table).offset(key as isize)).valid == 0 as libc::c_int {
        ...
        while !entry.is_null() {
                ...
                && xmlStrEqual((*entry).name3, name3) != 0
            {
                ...
                (*entry).payload = 0 as *mut libc::c_void;
                if ((*table).dict).is_null() {
