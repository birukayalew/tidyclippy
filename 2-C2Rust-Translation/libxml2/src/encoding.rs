    ...
    if xmlCharEncodingAliases.is_null() {
        ...
        xmlCharEncodingAliases = xmlMalloc
            ...
            (xmlCharEncodingAliasesMax as libc::c_ulong)
                .wrapping_mul(
                    ::core::mem::size_of::<xmlCharEncodingAlias>() as libc::c_ulong,
                ),
...
pub unsafe extern "C" fn xmlNewCharEncodingHandler(
    ...
    handler = xmlMalloc
        .expect(
            "non-null function pointer",
        )(::core::mem::size_of::<xmlCharEncodingHandler>() as libc::c_ulong)
    ...
    (*handler).name = up;
    (*handler).iconv_in = 0 as *mut libc::c_void;
    (*handler).iconv_out = 0 as *mut libc::c_void;
...
pub unsafe extern "C" fn xmlInitCharEncodingHandlers() {
    ...
    handlers = xmlMalloc
        ...
        (50 as libc::c_int as libc::c_ulong)
            .wrapping_mul(
                ::core::mem::size_of::<xmlCharEncodingHandlerPtr>() as libc::c_ulong,
            ),
...
pub unsafe extern "C" fn xmlRegisterCharEncodingHandler(
    ...
    if nbCharEncodingHandler >= 50 as libc::c_int {
        xmlEncodingErr(
...
pub unsafe extern "C" fn xmlFindCharEncodingHandler(
        ...
        && icv_out != -(1 as libc::c_int) as iconv_t
    {
        ...
        return enc;
    } else if icv_in != -(1 as libc::c_int) as iconv_t
        || icv_out != -(1 as libc::c_int) as iconv_t
    {
        xmlEncodingErr(
            XML_ERR_INTERNAL_ERROR,
...
unsafe extern "C" fn xmlIconvWrapper(
    ...
    ret = iconv(
        cd,
        &mut icv_in as *mut *const libc::c_char as *mut *mut libc::c_char,
        &mut icv_inlen,
        &mut icv_out,
        &mut icv_outlen,
    ) as libc::c_int;
    *inlen = (*inlen as size_t).wrapping_sub(icv_inlen) as libc::c_int as libc::c_int;
    *outlen = (*outlen as size_t).wrapping_sub(icv_outlen) as libc::c_int as libc::c_int;
    if icv_inlen != 0 as libc::c_int as size_t || ret == -(1 as libc::c_int) {
...
pub unsafe extern "C" fn xmlCharEncFirstLineInt(
    ...
    (*out).use_0 = ((*out).use_0).wrapping_add(written as libc::c_uint);
...
pub unsafe extern "C" fn xmlCharEncFirstLineInput(
    ...
    if toconv == 0 as libc::c_int as size_t {
        return 0 as libc::c_int;
    }
    written = (xmlBufAvail(out)).wrapping_sub(1 as libc::c_int as size_t);
    if len >= 0 as libc::c_int {
        if toconv > len as libc::c_uint as size_t {
            toconv = len as size_t;
        }
    } else if toconv > 180 as libc::c_int as size_t {
        toconv = 180 as libc::c_int as size_t;
    }
    if toconv * 2 as libc::c_int as size_t >= written {
        xmlBufGrow(out, (toconv * 2 as libc::c_int as size_t) as libc::c_int);
        written = (xmlBufAvail(out)).wrapping_sub(1 as libc::c_int as size_t);
    }
    if written > 360 as libc::c_int as size_t {
...
pub unsafe extern "C" fn xmlCharEncInput(
    ...
    if toconv > (64 as libc::c_int * 1024 as libc::c_int) as size_t
    ...
    if written > (128 as libc::c_int * 1024 as libc::c_int) as size_t
        && flush == 0 as libc::c_int
    {
...
pub unsafe extern "C" fn xmlCharEncOutput(
    ...
    let mut ret: libc::c_int = 0;
    let mut written: size_t = 0;
    let mut writtentot: size_t = 0 as libc::c_int as size_t;
    let mut toconv: size_t = 0;
    let mut c_in: libc::c_int = 0;
    let mut c_out: libc::c_int = 0;
    let mut in_0: xmlBufPtr = 0 as *mut xmlBuf;
    ...
    loop {
        ...
        if toconv * 4 as libc::c_int as size_t >= written {
            xmlBufGrow(out, (toconv * 4 as libc::c_int as size_t) as libc::c_int);
            written = (xmlBufAvail(out)).wrapping_sub(1 as libc::c_int as size_t);
        }
        if written > (256 as libc::c_int * 1024 as libc::c_int) as size_t {
        ...
        xmlBufShrink(in_0, c_in as size_t);
        xmlBufAddLen(out, c_out as size_t);
        writtentot = writtentot.wrapping_add(c_out as size_t);
        if ret == -(1 as libc::c_int) {
        ...
        if ret >= 0 as libc::c_int {
            output = output.offset(ret as isize);
        }
        match ret {
            -4 => {
                xmlEncodingErr(
            ...
            -2 => {
                ...
                charrefLen = snprintf(
                    ...
                    ::core::mem::size_of::<[xmlChar; 20]>() as libc::c_ulong,
                    b"&#%d;\0" as *const u8 as *const libc::c_char,
                    cur,
                );
                xmlBufShrink(in_0, len as size_t);
                xmlBufGrow(out, charrefLen * 4 as libc::c_int);
                c_out = (xmlBufAvail(out)).wrapping_sub(1 as libc::c_int as size_t)
                    as libc::c_int;
                c_in = charrefLen;
                ret = xmlEncOutputChunk(
                    (*output).encoder,
...
pub unsafe extern "C" fn xmlCharEncCloseFunc(
    ...
    if tofree != 0 {
        ...
        (*handler).name = 0 as *mut libc::c_char;
