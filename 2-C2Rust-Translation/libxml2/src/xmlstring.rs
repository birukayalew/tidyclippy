...
pub unsafe extern "C" fn xmlStrcmp(
    ...
    loop {
        let fresh0 = str1;
        str1 = str1.offset(1);
        tmp = *fresh0 as libc::c_int - *str2 as libc::c_int;
...
pub unsafe extern "C" fn xmlStrcasecmp(
    ...
    loop {
        let fresh9 = str1;
        str1 = str1.offset(1);
        tmp = casemap[*fresh9 as usize] as libc::c_int
...
pub unsafe extern "C" fn xmlStrncat(
    ...
    ret = xmlRealloc
        ...
        ((size + len + 1 as libc::c_int) as libc::c_ulong)
            .wrapping_mul(::core::mem::size_of::<xmlChar>() as libc::c_ulong),
    ) as *mut xmlChar;
    if ret.is_null() {
        xmlErrMemory(0 as xmlParserCtxtPtr, 0 as *const libc::c_char);
            ...
            if !(*len < 1 as libc::c_int) {
                ...
                if c & 0x80 as libc::c_int as libc::c_uint != 0 {
                    if *len < 2 as libc::c_int {
                        current_block = 12343893343324797886;
...
pub unsafe extern "C" fn xmlUTF8Strsize(
    ...
    return ptr.offset_from(utf) as libc::c_long as libc::c_int;
}
#[no_mangle]
        ...
        (*__xmlGenericError())
            ...
            *__xmlGenericErrorContext(),
            ...
            (len + 1 as libc::c_int) as libc::c_long
                * ::core::mem::size_of::<xmlChar>() as libc::c_ulong as libc::c_long,
        );
        return 0 as *mut xmlChar;
    }
    memcpy(
    ...
    loop {
        ...
        if ch as libc::c_int & 0x80 as libc::c_int != 0 {
            ...
            loop {
                ...
                let fresh17 = utf;
                utf = utf.offset(1);
                if *fresh17 as libc::c_int & 0xc0 as libc::c_int != 0x80 as libc::c_int {
                    return 0 as *const xmlChar;
                }
            }
        }
    ...
    loop {
        ...
        if ch as libc::c_int & 0x80 as libc::c_int != 0 {
            ...
            loop {
                ...
                let fresh18 = utf;
                utf = utf.offset(1);
                if *fresh18 as libc::c_int & 0xc0 as libc::c_int != 0x80 as libc::c_int {
                    return -(1 as libc::c_int);
                }
            }
        }
...
pub unsafe extern "C" fn xmlEscapeFormatString(
    ...
    msgPtr = *msg;
    while *msgPtr as libc::c_int != '\0' as i32 {
    ...
    resultLen = msgLen.wrapping_add(count).wrapping_add(1 as libc::c_int as size_t);
    result = xmlMallocAtomic
        .expect(
            "non-null function pointer",
        )(resultLen.wrapping_mul(::core::mem::size_of::<xmlChar>() as libc::c_ulong))
        as *mut xmlChar;
    if result.is_null() {
    ...
    *result
        .offset(
            resultLen.wrapping_sub(1 as libc::c_int as size_t) as isize,
        ) = '\0' as i32 as xmlChar;
    xmlFree.expect("non-null function pointer")(*msg as *mut libc::c_void);
    *msg = result;
    return *msg;
