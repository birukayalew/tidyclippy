...
pub unsafe extern "C" fn initGenericErrorDefaultFunc(
    ...
    if handler.is_null() {
        let ref mut fresh0 = *__xmlGenericError();
        *fresh0 = Some(
...
pub unsafe extern "C" fn xmlSetGenericErrorFunc(
    ...
    let ref mut fresh2 = *__xmlGenericErrorContext();
    *fresh2 = ctx;
    if handler.is_some() {
        let ref mut fresh3 = *__xmlGenericError();
        *fresh3 = handler;
    } else {
...
pub unsafe extern "C" fn xmlSetStructuredErrorFunc(
    ...
    let ref mut fresh5 = *__xmlStructuredErrorContext();
    *fresh5 = ctx;
    let ref mut fresh6 = *__xmlStructuredError();
    *fresh6 = handler;
}
#[no_mangle]
...
unsafe extern "C" fn xmlParserPrintFileContextInternal(
    ...
    loop {
        let fresh7 = n;
        n = n.wrapping_add(1);
        if !((fresh7 as libc::c_ulong)
            < (::core::mem::size_of::<[xmlChar; 81]>() as libc::c_ulong)
...
pub unsafe extern "C" fn xmlReportError(
    ...
    if domain == XML_FROM_XPATH as libc::c_int && !((*err).str1).is_null()
        && (*err).int1 >= 0 as libc::c_int && (*err).int1 < 100 as libc::c_int
        && (*err).int1 < xmlStrlen((*err).str1 as *const xmlChar)
    {
        ...
        let fresh12 = i;
        i = i + 1;
        buf[fresh12 as usize] = '^' as i32 as xmlChar;
...
pub unsafe extern "C" fn xmlResetError(mut err: xmlErrorPtr) {
    ...
    memset(
        ...
        0 as libc::c_int,
        ::core::mem::size_of::<xmlError>() as libc::c_ulong,
    );
